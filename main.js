...
const generateHTML = (results) => {
    ...
    /*const lowestCalorieRecipe = results.reduce((lowest, result) =>{
        return result.recipe.calories < lowest.recipe.calories ? result.recipe : lowest.recipe;
    });*/

    const lowestCalorieResult = results.reduce((lowest, result) => {
        console.log("processing recipe", lowest, result);
        return result.recipe.calories < lowest.recipe.calories ? result : lowest;
    }, DUMMY_RECIPE);
    console.log("lowest",lowestCalorieResult);



    results.map((result, index) => {
        const itemClasses = result.recipe.calories === lowestCalorieResult.recipe.calories ? 'item special' : 'item';
